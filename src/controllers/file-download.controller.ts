...
import {ImagenRepository, UsuarioRepository} from '../repositories';
...
export class FileDownloadController {
  ...
  private GetFolderPathByType(type: number) {
    ...
    switch (type) {
      ...
      // product
      case 2:
        filePath = path.join(__dirname, UploadFilesKeys.VIDEOJUEGO_IMAGE_PATH);
        break;
      /*// advertising
      case 3:
  ...
  private async GetFilenameById(type: number, recordId: string) {
    ...
    switch (type) {
      /* // customer
      case 1:
        const customer: Usuario = await this.customerRepository.findById(recordId);
        fileName = customer.profilePhoto ?? '';
        break;*/
      // product
      case 2:
        const image: Imagen = await this.imageRepository.findById(recordId);
        fileName = image.path;
        break;
      // advertising
      /*case 3:
